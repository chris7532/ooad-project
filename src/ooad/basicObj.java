...
public class basicObj extends JPanel implements MouseListener, MouseMotionListener {
	...
	public void chooseStartPort(MouseEvent e) {
		...
		for(int j=0;j<4;j++) {
			distanceToPort[j] = Math.sqrt( Math.pow(portArray[j].getLocation().x-startPoint.x, 2) + Math.pow(portArray[j].getLocation().y-startPoint.y, 2));
		}
		double min = 10000;
		for(int i=0;i<4;i++) {
	...
	/*
	public void chooseEndPort(MouseEvent e) {
		//Point endPoint = e.getPoint(); //relative to basicObj panel
		Point endPoint = MouseInfo.getPointerInfo().getLocation();
        SwingUtilities.convertPointFromScreen(endPoint, canvas);
		for(int j=0;j<4;j++) {
			distanceToPort[j] = Math.sqrt( Math.pow(portArray[j].getLocation().x- endPoint.x, 2) + Math.pow(portArray[j].getLocation().y-endPoint.y, 2));
		}
		double min = 10000;
		for(int i=0;i<4;i++) {
			if(distanceToPort[i] < min) {
	...
	public void addLine() {
		...
		if(canvas.frame.currentBtn == canvas.frame.assButton)
			canvas.line = new associationLine(canvas.tempStartPoint, canvas.tempEndPoint, canvas.tempStartPort, canvas.tempEndPort); 
		if(canvas.frame.currentBtn == canvas.frame.genButton)
			canvas.line = new generalizationLine(canvas.tempStartPoint, canvas.tempEndPoint, canvas.tempStartPort, canvas.tempEndPort);
		if(canvas.frame.currentBtn == canvas.frame.comButton)
			canvas.line = new compositionLine(canvas.tempStartPoint, canvas.tempEndPoint, canvas.tempStartPort, canvas.tempEndPort);
    ...
    public void mouseReleased(MouseEvent e) {
		if(canvas.frame.currentBtn == canvas.frame.assButton || canvas.frame.currentBtn == canvas.frame.genButton
			...
			for(int j=0;j<4;j++) {
				lineEndBlock.distanceToPort[j] = Math.sqrt( Math.pow(lineEndBlock.portArray[j].block.getLocation().x + lineEndBlock.portArray[j].getLocation().x - endPoint.x, 2) 
						+ Math.pow(lineEndBlock.portArray[j].block.getLocation().y + lineEndBlock.portArray[j].getLocation().y - endPoint.y, 2));
			}
			double min = 10000;
			for(int i=0;i<4;i++) {
